...
from PyQt4.QtGui import QApplication, QMainWindow, QWidget, QVBoxLayout, QLabel
from PyQt4.QtGui import QFileDialog, QAction, QTextBrowser, QHBoxLayout, QCheckBox
from PyQt4.QtGui import QIcon, QDoubleSpinBox, QPushButton, QLineEdit, QIntValidator
from PyQt4.QtGui import QGridLayout, QFrame, QMessageBox, QDoubleValidator, QTextEdit
from PyQt4.QtGui import QDesktopWidget, QFont, QCursor

from PyQt4.QtCore import QT_VERSION_STR, PYQT_VERSION_STR, QFileInfo
from PyQt4.QtCore import  SIGNAL, Qt, SLOT, QByteArray, QSettings, QFile, QPoint
...
class Form(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.connect(self.peaks_cb, SIGNAL("stateChanged(int)"),
                         self.addPeakLabels)
        self.connect(self.spinBox_tresh, SIGNAL("valueChanged(double)"),
                         self.addPeakLabels)
        self.connect(self.pushButton_calibrate, SIGNAL('clicked()'),    
                     self.quickCalibrateHistogram)
        self.connect(self.pbCalibrateMorePeaks, SIGNAL('clicked()'),    
                     self.acquirePeakList)        
        self.connect(self.le_ch1, SIGNAL('textChanged(QString)'),
                     self.plotVerticalLines)
        self.connect(self.le_ch2, SIGNAL('textChanged(QString)'),
                     self.plotVerticalLines)
        self.connect(self.pushButton_maps, SIGNAL('clicked()'),
                     self.mappingFewHighestPeaks)
        self.connect(self.pushButton_mapsZ, SIGNAL('clicked()'),
                     self.mappingFromFile)
        self.connect(self.pushButton_mapOnClick, SIGNAL('clicked()'),
                     self.mappingDataBetweenGreenRed)

        self.connect(self.pushButton_commands, SIGNAL('clicked()'),
                     self.executeCommands)
        self.connect(self.pbNewton, SIGNAL('clicked()'), self.deletePlottedFit)
...
#********************************************* SLOTS
    ...
    def fitDlg(self):
        ...
        self.connect(self.fitDialog, SIGNAL("changed"), self.fit)
        self.fitDialog.show()
    ...
    def timeOfFlightCalculationDlg(self):
        ...
        self.connect(dlg, SIGNAL("changed"),self.timeOfFlightCalculation)
        dlg.show()
...
#***********************************************Metode povezane s kalibracijo
    ...
    def acquirePeakList(self):
        ...
        self.connect(self.dialog, SIGNAL("changed"),self.calibrateHistogramPeakList)
        self.connect(self.dialog.pbFillFromRedLine, SIGNAL("clicked()"),self.fill)
        self.dialog.show()
