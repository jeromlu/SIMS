...
from PyQt4.QtCore import QThread, pyqtSignal, SIGNAL, QMutex
...
class imzMLThread(QThread):
    notifyProgress = pyqtSignal(int)
    
    def __init__(self, parent = None):
        super(imzMLThread, self).__init__(parent = None)
        self.stopped = False
        self.completed = False
        self.mutex = QMutex()

    def initialize(self, data, pixels, binning,  UUID, calParam = (0,1), fname = None, massRange = [1,500], formatN="float32", reducedFormat = False):
        self.completed=False
        self.data = data
        self.pixels = pixels
        self.binning = binning
        self.UUID = UUID
        self.calParam = calParam
        self.fname = fname
        self.massRange = massRange
        self.formatN = formatN
        self.reducedFormat = reducedFormat
...
#        self.wait()    
    ...
    def createIBD(self, data):
        """tole je se treba vkljucit QFileInfo(QFileInfo(path).path()).fileName()
        mam dva nacina, en je res imzML en je pa moj fajl """
        
        if self.fname:
            if self.reducedFormat:
                OUT = open(self.fname+"\\mojFormat"+str(self.pixels)+".ibd","bw")
            else:
                OUT = open(self.fname+"\\generiranXML"+str(self.pixels)+".ibd","bw")
                OUT.write(self.UUID.bytes)
        else:
            return
        massRange=np.array(self.massRange)

        countRange = self.calParam[0]+self.calParam[1]*np.sqrt(massRange)+ \
                self.calParam[2]*massRange+ np.sqrt(massRange)**3*self.calParam[3]
        maxVal = 32768
        minVal = 0
        intSize = maxVal / self.pixels
        intervals = np.arange(minVal,maxVal,intSize)
        count=1
        binsN = np.arange(countRange[0],countRange[1]+1,(countRange[1]-countRange[0])/self.binning)
        print(binsN)
        tmax = (self.pixels**2/100)
        ...
        for i in range(self.pixels):
                    ...
                    for j in range(self.pixels):
                        ...
                        count = count + 1
                        #dataM= (((data['time'][mask])-self.calParam[0])/self.calParam[1])**2
                        
                        if self.reducedFormat:
                            H, bins = np.histogram(data['time'][mask], bins=range(11250))
                        else:                            
                            H, bins = np.histogram(data['time'][mask], bins=binsN)
                        
                        if not self.reducedFormat:
                            if not self.calParam:
                                (bins[1:]).astype('float32',copy=False).tofile(OUT)
                            else:
                                bins= (bins[:-1] + bins[1:])/2
                                mass = ((bins[1:]-self.calParam[0])/self.calParam[1])**2
                                (mass).astype('float32',copy=False).tofile(OUT)
                            H.astype('float32',copy=False).tofile(OUT)
                        else:
                            if i ==0 and j== 0:
                                bins= (bins[:-1] + bins[1:])/2
                                mass = spectrumViewer.Newton(bins,self.calParam)
                                (mass**2).astype('float32',copy=False).tofile(OUT)
                            H.astype('int16',copy=False).tofile(OUT)
                        if count % round(tmax) ==0:
                            self.notifyProgress.emit(round(int(count/self.pixels**2*100)))
        if OUT:
            OUT.close()
